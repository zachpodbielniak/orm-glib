              ...
              GError        **error)
{
    ...
    /* Keep the first migration in flight while the other process starts. */
    g_usleep (200000);
...
test_migration (gconstpointer data)
{
    ...
    g_signal_connect (migrator, "migration-applied", G_CALLBACK (count_applied), &applied);
    g_signal_connect (migrator, "migration-failed", G_CALLBACK (count_failed), &failed);
    ...
    {
        GPid children[2];
        gchar *args[] = { (gchar *) test_program, "--worker", (gchar *) test_url, NULL };
        gint i;
        gint status;

        for (i = 0; i < 2; i++)
        {
            g_assert_true (g_spawn_async (NULL, args, NULL, G_SPAWN_DO_NOT_REAP_CHILD,
    ...
    else if (failing)
    {
        ...
        g_assert_cmpint (scalar (connection, "SELECT COUNT(*) FROM migration_items"), ==, 0);
        if (orm_engine_get_dialect_type (engine) != ORM_DIALECT_MYSQL)
        {
            ...
            g_assert_cmpint (scalar (connection, "SELECT COUNT(*) FROM migration_partial"), ==, 0);
        return;
    }
    else
    {
    ...
    g_assert_cmpint (scalar (connection, "SELECT COUNT(*) FROM schema_migrations"), ==, 2);
    g_assert_cmpint (scalar (connection, "SELECT COUNT(*) FROM migration_items"), ==, 1);
    g_assert_cmpint (scalar (connection, "SELECT id FROM migration_items"), ==, 7);
    if (callback)
    {
        g_autoptr(OrmResult) result = orm_connection_query (connection,
        ...
        sql_result = orm_connection_query (connection, "SELECT * FROM migration_sql_items", &error);
        g_assert_no_error (error);
...
main (gint argc, gchar **argv)
{
    ...
    if (argc == 3 && g_str_equal (argv[1], "--worker"))
    {
        ...
        g_autoptr(OrmConnection) connection = orm_engine_connect (engine, &error);
        g_autoptr(OrmMigrator) migrator = new_migrator (connection, TRUE, FALSE, FALSE);
        g_assert_no_error (error);
        g_assert_true (orm_migrator_up (migrator, 0, &error));
        g_assert_no_error (error);
        return 0;
