...
/*
 * OrmPostgresDdlCompiler - PostgreSQL DDL compiler implementation.
 *
 * Generates PostgreSQL-specific DDL statements for creating and
 * modifying database schema.
 */
struct _OrmPostgresDdlCompiler
{
    GObject parent_instance;
};

static void orm_postgres_ddl_compiler_iface_init (OrmDdlCompilerInterface *iface);

G_DEFINE_TYPE_WITH_CODE (OrmPostgresDdlCompiler, orm_postgres_ddl_compiler, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (ORM_TYPE_DDL_COMPILER,
                                                orm_postgres_ddl_compiler_iface_init))

/*
 * Helper: Quote an identifier for PostgreSQL.
...
get_column_type_sql (OrmColumn *column)
{
    ...
    return orm_sql_type_get_name (type, ORM_DIALECT_POSTGRES);
}
 ...
 */
static gchar *
orm_postgres_ddl_compiler_compile_column_definition (OrmDdlCompiler *compiler,
                                                      OrmColumn      *column)
{
    GString *result;
    ...
    /*
     * For auto-increment columns, use SERIAL/BIGSERIAL instead of type.
     * SERIAL = INTEGER + sequence + DEFAULT nextval(...)
     * BIGSERIAL = BIGINT + sequence + DEFAULT nextval(...)
     */
    if (is_autoincrement)
    {
        type_sql = get_column_type_sql (column);
        if (g_strcmp0 (type_sql, "BIGINT") == 0)
        {
            g_string_append (result, "BIGSERIAL");
        }
        else
        {
            g_string_append (result, "SERIAL");
        }
    }
    else
    {
 ...
 */
static gchar *
orm_postgres_ddl_compiler_compile_primary_key (OrmDdlCompiler *compiler,
                                                OrmPrimaryKey  *pk)
{
    GString *result;
    GList *columns;
 ...
 */
static gchar *
orm_postgres_ddl_compiler_compile_foreign_key (OrmDdlCompiler *compiler,
                                                OrmForeignKey  *fk)
{
    GString *result;
    GList *columns;
 ...
 */
static gchar *
orm_postgres_ddl_compiler_compile_create_table (OrmDdlCompiler *compiler,
                                                 OrmTable       *table,
                                                 gboolean        if_not_exists)
{
    ...
    for (l = columns; l != NULL; l = l->next)
    {
        ...
        col_def = orm_postgres_ddl_compiler_compile_column_definition (compiler, column);
        g_string_append (result, "    ");
    ...
    if (pk != NULL)
    {
        ...
        if (pk_column_count > 1)
        {
            ...
            g_string_append (result, ",\n    ");
            pk_sql = orm_postgres_ddl_compiler_compile_primary_key (compiler, pk);
    ...
    for (l = foreign_keys; l != NULL; l = l->next)
    {
        ...
        g_string_append (result, ",\n    ");
        fk_sql = orm_postgres_ddl_compiler_compile_foreign_key (compiler, fk);
 ...
 */
static gchar *
orm_postgres_ddl_compiler_compile_drop_table (OrmDdlCompiler *compiler,
                                               OrmTable       *table,
                                               gboolean        if_exists,
                                               gboolean        cascade)
{
    ...
    /* PostgreSQL supports CASCADE on DROP TABLE */
    if (cascade)
    {
        g_string_append (result, " CASCADE");
    }
 ...
 */
static gchar *
orm_postgres_ddl_compiler_compile_create_index (OrmDdlCompiler *compiler,
                                                 OrmIndex       *index,
                                                 OrmTable       *table,
                                                 gboolean        if_not_exists)
{
 ...
 */
static gchar *
orm_postgres_ddl_compiler_compile_drop_index (OrmDdlCompiler *compiler,
                                               OrmIndex       *index,
                                               OrmTable       *table,
                                               gboolean        if_exists)
{
    ...
    (void) compiler;
    (void) table; /* PostgreSQL doesn't need table name for DROP INDEX */
...
static void
orm_postgres_ddl_compiler_iface_init (OrmDdlCompilerInterface *iface)
{
    iface->compile_create_table = orm_postgres_ddl_compiler_compile_create_table;
    iface->compile_drop_table = orm_postgres_ddl_compiler_compile_drop_table;
    iface->compile_column_definition = orm_postgres_ddl_compiler_compile_column_definition;
    iface->compile_primary_key = orm_postgres_ddl_compiler_compile_primary_key;
    iface->compile_foreign_key = orm_postgres_ddl_compiler_compile_foreign_key;
    iface->compile_create_index = orm_postgres_ddl_compiler_compile_create_index;
    iface->compile_drop_index = orm_postgres_ddl_compiler_compile_drop_index;
}

static void
orm_postgres_ddl_compiler_class_init (OrmPostgresDdlCompilerClass *klass)
{
    (void) klass;
}

static void
orm_postgres_ddl_compiler_init (OrmPostgresDdlCompiler *self)
{
    (void) self;
}

/**
 * orm_postgres_ddl_compiler_new:
 *
 * Creates a new PostgreSQL DDL compiler.
 *
 * Returns: (transfer full): A new #OrmPostgresDdlCompiler
 */
OrmPostgresDdlCompiler *
orm_postgres_ddl_compiler_new (void)
{
    return g_object_new (ORM_TYPE_POSTGRES_DDL_COMPILER, NULL);
}
