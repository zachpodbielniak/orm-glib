...
/*
 * OrmMysqlDdlCompiler - MySQL DDL compiler implementation.
 *
 * Generates MySQL-specific DDL statements for creating and
 * modifying database schema.
 */
struct _OrmMysqlDdlCompiler
{
    GObject parent_instance;
};

static void orm_mysql_ddl_compiler_iface_init (OrmDdlCompilerInterface *iface);

G_DEFINE_TYPE_WITH_CODE (OrmMysqlDdlCompiler, orm_mysql_ddl_compiler, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (ORM_TYPE_DDL_COMPILER,
                                                orm_mysql_ddl_compiler_iface_init))

/*
 * Helper: Quote an identifier for MySQL.
...
get_column_type_sql (OrmColumn *column)
{
    ...
    return orm_sql_type_get_name (type, ORM_DIALECT_MYSQL);
}
 ...
 */
static gchar *
orm_mysql_ddl_compiler_compile_column_definition (OrmDdlCompiler *compiler,
                                                   OrmColumn      *column)
{
    GString *result;
    ...
    is_pk = orm_column_get_primary_key (column);
    is_autoincrement = orm_column_get_autoincrement (column);

    /* Type */
    type_sql = get_column_type_sql (column);
    g_string_append (result, type_sql);

    /* NOT NULL (before AUTO_INCREMENT) */
    if (!orm_column_get_nullable (column) && !is_pk)
    {
        g_string_append (result, " NOT NULL");
    }

    /* AUTO_INCREMENT (MySQL-specific, implies NOT NULL) */
    if (is_autoincrement)
    {
        g_string_append (result, " AUTO_INCREMENT");
    }
    ...
    /* UNIQUE */
    if (orm_column_get_unique (column) && !is_pk)
    {
        g_string_append (result, " UNIQUE");
    }

    /* DEFAULT (not for AUTO_INCREMENT columns) */
    if (!is_autoincrement)
    {
 ...
 */
static gchar *
orm_mysql_ddl_compiler_compile_primary_key (OrmDdlCompiler *compiler,
                                             OrmPrimaryKey  *pk)
{
    GString *result;
    GList *columns;
    ...
    /* Optional constraint name (MySQL supports named constraints) */
 ...
 */
static gchar *
orm_mysql_ddl_compiler_compile_foreign_key (OrmDdlCompiler *compiler,
                                             OrmForeignKey  *fk)
{
    GString *result;
    GList *columns;
    ...
    case ORM_FK_SET_DEFAULT:
        /* MySQL doesn't support SET DEFAULT for InnoDB */
        g_string_append (result, " ON DELETE NO ACTION");
        break;
    ...
    case ORM_FK_SET_DEFAULT:
        /* MySQL doesn't support SET DEFAULT for InnoDB */
        g_string_append (result, " ON UPDATE NO ACTION");
        break;
    case ORM_FK_RESTRICT:
 ...
 */
static gchar *
orm_mysql_ddl_compiler_compile_create_table (OrmDdlCompiler *compiler,
                                              OrmTable       *table,
                                              gboolean        if_not_exists)
{
    ...
    for (l = columns; l != NULL; l = l->next)
    {
        ...
        col_def = orm_mysql_ddl_compiler_compile_column_definition (compiler, column);
        g_string_append (result, "    ");
    ...
    if (pk != NULL)
    {
        ...
        if (pk_column_count > 1)
        {
            ...
            g_string_append (result, ",\n    ");
            pk_sql = orm_mysql_ddl_compiler_compile_primary_key (compiler, pk);
    ...
    for (l = foreign_keys; l != NULL; l = l->next)
    {
        ...
        g_string_append (result, ",\n    ");
        fk_sql = orm_mysql_ddl_compiler_compile_foreign_key (compiler, fk);
        g_string_append (result, fk_sql);
    }

    g_string_append (result, "\n)");

    /* Default engine for foreign key support */
    g_string_append (result, " ENGINE=InnoDB");
 ...
 */
static gchar *
orm_mysql_ddl_compiler_compile_drop_table (OrmDdlCompiler *compiler,
                                            OrmTable       *table,
                                            gboolean        if_exists,
                                            gboolean        cascade)
{
    ...
    (void) compiler;
    (void) cascade; /* MySQL doesn't support CASCADE on DROP TABLE */
 ...
 */
static gchar *
orm_mysql_ddl_compiler_compile_create_index (OrmDdlCompiler *compiler,
                                              OrmIndex       *index,
                                              OrmTable       *table,
                                              gboolean        if_not_exists)
{
    ...
    (void) compiler;
    (void) if_not_exists; /* MySQL doesn't support IF NOT EXISTS for CREATE INDEX */
 ...
 */
static gchar *
orm_mysql_ddl_compiler_compile_drop_index (OrmDdlCompiler *compiler,
                                            OrmIndex       *index,
                                            OrmTable       *table,
                                            gboolean        if_exists)
{
    ...
    (void) compiler;
    (void) if_exists; /* MySQL doesn't support IF EXISTS for DROP INDEX */
...
static void
orm_mysql_ddl_compiler_iface_init (OrmDdlCompilerInterface *iface)
{
    iface->compile_create_table = orm_mysql_ddl_compiler_compile_create_table;
    iface->compile_drop_table = orm_mysql_ddl_compiler_compile_drop_table;
    iface->compile_column_definition = orm_mysql_ddl_compiler_compile_column_definition;
    iface->compile_primary_key = orm_mysql_ddl_compiler_compile_primary_key;
    iface->compile_foreign_key = orm_mysql_ddl_compiler_compile_foreign_key;
    iface->compile_create_index = orm_mysql_ddl_compiler_compile_create_index;
    iface->compile_drop_index = orm_mysql_ddl_compiler_compile_drop_index;
}

static void
orm_mysql_ddl_compiler_class_init (OrmMysqlDdlCompilerClass *klass)
{
    (void) klass;
}

static void
orm_mysql_ddl_compiler_init (OrmMysqlDdlCompiler *self)
{
    (void) self;
}

/**
 * orm_mysql_ddl_compiler_new:
 *
 * Creates a new MySQL DDL compiler.
 *
 * Returns: (transfer full): A new #OrmMysqlDdlCompiler
 */
OrmMysqlDdlCompiler *
orm_mysql_ddl_compiler_new (void)
{
    return g_object_new (ORM_TYPE_MYSQL_DDL_COMPILER, NULL);
}
