 ...
 * often generated by something like `openssl rand -base64`, which produces
 * '/' and '+' routinely, and both are structural in a URL. Without decoding
 * here, such a password reaches the server as the literal "%2F..." and
 * authentication fails with a message that says nothing about why.
 *
 * Text that is not valid percent-encoding is taken literally rather than
 * rejected, so a password containing a bare '%' still works.
 *
 * Returns: (transfer full) (nullable): the decoded copy
 */
static gchar *
orm_uri_component_unescape (const gchar *text,
                            gsize        length)
{
         ...
         */
        {
            ...
            if (at_sign != NULL)
            {
                ...
                if (colon != NULL && colon < at_sign)
                {
                    ...
                    self->username = orm_uri_component_unescape (p, colon - p);
                    self->password = orm_uri_component_unescape (
                        colon + 1, at_sign - colon - 1);
                }
                else
                {
                    /* Only user, no password */
                    self->username = orm_uri_component_unescape (p, at_sign - p);
                    self->password = NULL;
                }
