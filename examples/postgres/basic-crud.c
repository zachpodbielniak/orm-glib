 ...
 *
 * Example: Basic CRUD Operations with PostgreSQL
 * ===============================================
 *
 * This example demonstrates basic Create, Read, Update, Delete operations
 * using orm-glib with PostgreSQL.
 *
 * Prerequisites:
 *   make container-postgres-start
 *
 * Or set ORM_POSTGRES_URL environment variable to your PostgreSQL URL.
 */
      ...
      char **argv)
{
    ...
    g_print ("orm-glib PostgreSQL Basic CRUD Example\n");
    g_print ("======================================\n\n");
    ...
    /*
     * Step 1: Create an engine with PostgreSQL database
     */
    g_print ("1. Creating PostgreSQL engine...\n");
    engine = orm_engine_new (url, &error);
    if (engine == NULL)
    {
        g_printerr ("Failed to create engine: %s\n", error->message);
        g_printerr ("Make sure PostgreSQL is running: make container-postgres-start\n");
        return EXIT_FAILURE;
    }
