 ...
 *
 * Example: Foreign Key Relationships with PostgreSQL
 * ===================================================
 *
 * Prerequisites:
 *   make container-postgres-start
 */
...
#define DEFAULT_POSTGRES_URL "postgresql://orm_test:orm_test_pass@localhost:15432/orm_test"
...
main (int argc, char **argv)
{
    ...
    g_print ("orm-glib PostgreSQL Relationships Example\n");
    g_print ("==========================================\n\n");

    url = g_getenv ("ORM_POSTGRES_URL");
    if (url == NULL) url = DEFAULT_POSTGRES_URL;
    g_print ("Connecting to: %s\n\n", url);

    engine = orm_engine_new (url, &error);
    if (engine == NULL)
    {
        g_printerr ("Error: %s\nRun: make container-postgres-start\n", error->message);
        return 1;
    }
    ...
    {
        g_autoptr(ExamplePost) p1 = example_post_new ("PostgreSQL Tutorial", 1);
        g_autoptr(ExamplePost) p2 = example_post_new ("Advanced SQL", 1);
        g_autoptr(ExamplePost) p3 = example_post_new ("Database Design", 2);
